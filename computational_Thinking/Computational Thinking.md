...
1. 문자.
2. 정수 : 2의 보수로 저장
3. 실수 : 부호|지수|가수부 => 부호 (1) | 지수 (8) | 가수부(23)
...
+ O(n^2) : 선택정렬, 버블정렬, 삽입정렬, 다익스트라 최단경로 알고리즘
+ O(n^3) : 연쇄행렬곱, 모든 쌍 최단경로 알고리즘(floyd)
+ O(2^n) : 부분집합, 조합
+ O(n!) : 순열, 여행자문제(TSP)
...
나머지 : NP문제. 비결정적 다항식. 다항식 시간에 안풀린다. => 사용하려면 근사알고리즘 사용.
...
| ~      | 단항 연산자로서 피연산자의 모든 비트를 반전 ex) ~num         |
| <<     | 피연산자의 비트 열을 왼쪽으로 이동 ex) num << 2              |
| >>     | 피연산자의 비트 열을 오른쪽으로 이동 ex) num >> 2            |

+ `1 << n ` : 2 ^ n의 값을 갖는다. 모든 부분집합(powerset)
+ `i & (1 << j) ` : i의 j 번째 비트가 1인지 아닌지를 의미한다.
...
+ ![1553056855117](img/1553056855117.png)

+ 소수일 때에는 2씩을 곱해 정수부분이 1인지 0인지를 취한다.( 이 때에는 위에서부터 취한다.)
...
![1553060481427](img/1553060481427.png)
